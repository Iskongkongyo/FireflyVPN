package xyz.a202132.app.ui.theme

import androidx.compose.ui.graphics.Color

// Primary Colors - Gradient Blue/Purple Theme
val Primary = Color(0xFF6366F1)
val PrimaryVariant = Color(0xFF4F46E5)
val PrimaryDark = Color(0xFF3730A3)
val Secondary = Color(0xFF8B5CF6)
val SecondaryVariant = Color(0xFF7C3AED)

// Background Colors
val BackgroundDark = Color(0xFF0F172A)
val BackgroundDarkVariant = Color(0xFF1E293B)
val SurfaceDark = Color(0xFF1E293B)
val CardDark = Color(0xFF334155)

// Text Colors
val TextPrimary = Color(0xFFF8FAFC)
val TextSecondary = Color(0xFF94A3B8)
val TextTertiary = Color(0xFF64748B)

// Status Colors
val ConnectedGreen = Color(0xFF22C55E)
val ConnectedGreenGlow = Color(0xFF4ADE80)
val DisconnectedGray = Color(0xFF64748B)
val ConnectingYellow = Color(0xFFFACC15)
val ErrorRed = Color(0xFFEF4444)

// Button Colors
val ButtonActive = Color(0xFF6366F1)
val ButtonInactive = Color(0xFF475569)

// Latency Colors
val LatencyGood = Color(0xFF22C55E)
val LatencyMedium = Color(0xFFFACC15)
val LatencyBad = Color(0xFFEF4444)

// Misc
val Divider = Color(0xFF334155)
val Ripple = Color(0x2A6366F1)
val AccentGreen = Color(0xFF10B981)
val AccentBlue = Color(0xFF3B82F6)

// Light Theme Colors
val BackgroundLight = Color(0xFFFFFFFF)
val BackgroundLightVariant = Color(0xFFF1F5F9)
val SurfaceLight = Color(0xFFFFFFFF)
val CardLight = Color(0xFFF8FAFC)

val TextPrimaryLight = Color(0xFF0F172A)
val TextSecondaryLight = Color(0xFF475569)
val TextTertiaryLight = Color(0xFF64748B)

val DividerLight = Color(0xFFE2E8F0)
val ButtonInactiveLight = Color(0xFFCBD5E1)
